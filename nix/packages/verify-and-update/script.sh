...
check_if_update_needed() {
  local -r verified_flake="$1"
  local current_system new_system_path hostname

  # Get current system
  if ! current_system=$(get_current_system); then
    log "First deployment, update needed"
    return 0
  fi
  ...
  log "Evaluating configuration without building (fast check)..."

  # Evaluate the output path without building
  case "$(uname -s)" in
    Darwin)
      new_system_path=$(nix eval --raw "$verified_flake#darwinConfigurations.$hostname.config.system.build.toplevel.outPath" 2>/dev/null)
      ;;
    Linux)
      new_system_path=$(nix eval --raw "$verified_flake#nixosConfigurations.$hostname.config.system.build.toplevel.outPath" 2>/dev/null)
      ;;
  esac

  if [[ -z "$new_system_path" ]]; then
    log "Warning: Could not evaluate output path, proceeding with build"
    return 0
  fi

  # Compare paths
  if [[ "$current_system" == "$new_system_path" ]]; then
    log "✓ System unchanged (output path identical)"
    log "  Current: $current_system"
    log "  Evaluated: $new_system_path"
    return 1
  fi

  log "System changes detected"
  log "  Current: $current_system"
  log "  New: $new_system_path"
  return 0
}
...
perform_update() {
  ...
  # Fast check: Evaluate output path without building
  if ! check_if_update_needed "$verified_flake"; then
    log "✓ No update needed, skipping expensive build"
    return 0
  fi
  ...
  case "$(uname -s)" in
    Linux)
      # Use 'boot' for NixOS (atomic, activates on next boot)
      ...
      ;;
    Darwin)
      # Use 'switch' for Darwin (activates immediately)
      log "Activating new configuration..."
      if ! "$rebuild_cmd" switch --flake "$verified_flake"; then
        log "✗ Failed to activate configuration"
        rm -f ./result
        return 1
      fi
      log "✓ Configuration activated successfully"
      ;;
  esac
