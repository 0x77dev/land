...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    # (claude-code/codex/pi); atuin's search hides them by default
    # ($all-user) and `--author '$all-agent'` surfaces them. `atuin hook
    # install` is upstream's own idempotent installer and edits mutable
    # agent state (~/.claude/settings.json, ~/.codex/hooks.json,
    # ~/.pi/agent/extensions/atuin.ts), so it runs at activation rather
    # than being templated by Nix.
