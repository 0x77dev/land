...
{
  # Nixvim pins its own tested nixpkgs; we intentionally build it against our
  # `unstable` (via the flake `follows`). Silence the resulting release-mismatch
  # check — the combination is deliberate and builds cleanly.
  ...
  extraConfigLua = ''
    ...
    vim.api.nvim_create_autocmd("FileType", {
      ...
      callback = function(event)
        -- Snacks.picker.get returns a list of matching pickers, not one.
        local ok, pickers = pcall(function()
          return Snacks.picker.get({ buf = event.buf })
        end)
        local picker = ok and pickers and pickers[1] or nil

        if not picker or picker.opts.source ~= "explorer" then
          return
        end
