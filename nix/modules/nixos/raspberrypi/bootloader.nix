...
let
  ...
  bootloader =
    ...
    pkgs.replaceVarsWith {
      src = ./scripts/generational/nixos-generations-builder.sh;
      isExecutable = true;
  ...
  kernelbootGenBuilder =
    ...
    pkgs.replaceVarsWith {
      src = ./scripts/generational/kernelboot-gen-builder.sh;
      isExecutable = true;
  ...
  deviceTree =
    ...
    pkgs.replaceVarsWith {
      src = ./scripts/generational/install-device-tree.sh;
      isExecutable = true;
  ...
  raspberryPiFirmware =
    ...
    pkgs.replaceVarsWith {
      src = ./scripts/generational/install-firmware.sh;
      isExecutable = true;
  ...
  kernelbootBuilder = import ./scripts/kernelboot-builder.nix {
    inherit pkgs;
    firmwareBuilder = firmwarePopulateCmd;
  };
  ubootBuilder = import ./scripts/uboot-builder.nix {
  ...
  populateFirmwareBuilder = import ./scripts/firmware-builder.nix {
    pkgs = pkgs.buildPackages;
    configTxt = cfg.configTxtPackage;
    firmware = cfg.firmwarePackage;
  };
  populateKernelbootBuilder = import ./scripts/kernelboot-builder.nix {
    pkgs = pkgs.buildPackages;
    firmwareBuilder = firmwarePopulateCmd;
  };
  populateUbootBuilder = import ./scripts/uboot-builder.nix {
      ...
      firmwarePackage = mkPackageOption pkgs "raspberrypifw" {
        ...
        extraDescription = ''
          ...
          - install device tree files from when
            `boot.loader.rpi.useGenerationDeviceTree == false`.
        '';
      };
      ...
      useGenerationDeviceTree = mkOption {
        ...
        description = ''
          ...
          - or from the vendor's firmware package set with
            `boot.loader.rpi.firmwarePackage` (when `false`)
  ...
  config = mkMerge [
    (mkIf cfg.enable {
      warnings = lib.optional (cfg.bootloader == "kernelboot") ''
        ...
        You're using boot.loader.rpi.bootloader = "${config.boot.loader.rpi.bootloader}",
      ...
      boot = {
        loader = {
          grub.enable = false;
          rpi = {
            firmwarePopulateCmd = populateCmds.${cfg.bootloader}.firmware;
            bootPopulateCmd = populateCmds.${cfg.bootloader}.boot;
          };
        };
      };
    })
    ...
    (mkIf (cfg.enable && (cfg.bootloader == "uboot")) {
      ...
      boot.loader.generic-extlinux-compatible = {
        ...
        inherit (cfg) useGenerationDeviceTree;
      };
