...
# shellcheck disable=SC2239,SC3030,SC3037,SC3043,SC3044,SC3054
shopt -s nullglob
...
if [ -n "$useVendorDeviceTree" ]; then
    printf "using vendor firmware from "
    dtb_path=$SRC_FIRMWARE_DIR
else
    printf "using default generation's kernel device tree binaries: "
