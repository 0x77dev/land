        ...
        let
          ...
          hermesVenv = upstream.hermesVenv.overrideAttrs (old: {
            postFixup = (old.postFixup or "") + ''
              ...
              source=$(readlink -f $site/hermes_cli)
              rm $site/hermes_cli
              cp -r --no-preserve=mode "$source" $site/hermes_cli
              rm -rf $site/hermes_cli/__pycache__
              patch -d $site -p1 < ${./patches/hermes-ephemeral-fallback.patch}
            '';
          });
        in
